...
│  ├ 不安要素.md                 ← チーム内の懸念事項メモ
│  └ 01_コーディングルール・命名規約.md  ← コーディング規約・命名規則の定義
```
...
## ✅ HTML/CSSコーディング時の必須ルール

HTML/CSSを書いたり更新したりするときは、必ず以下を行うこと：

1. `sankou/01_コーディングルール・命名規約.md` のルールに従ってコーディングする
2. 完了後は同ファイル末尾の **「コーディング完了チェックリスト」** を全項目確認する
3. 特に以下は見落としやすいので注意する：
   - `display: flex` / `display: grid` へのコメント
   - HTMLの重要箇所（セクション・flex・grid・position）へのコメント
...
- ブレイクポイント・PCの最大幅/最小幅 → ✅ PCファースト（max-width）・数値はデザインカンプに合わせる
- CSSクラス名の命名規則 → ✅ sankou/01_コーディングルール・命名規約.md に定義済み
